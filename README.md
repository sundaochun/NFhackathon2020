...
* Docker image: https://hub.docker.com/repository/docker/sundaochun/daochunproject  Note: 1）the docker image crashes at line 545 when I test it on a iMac pro (2017) with MacOS10.14.6, but smoothly on a PC server running windows10. Line 545,546 and 547 require huge compuatation resource and could take up to one hour to finish. The R code can run smoothly in Rstudio Version 1.2.5033 with R-4.0.2 on both of computers without the docker container.2) With unknow reason, when generating the GOsimilarity plot in pdf, the code lines 551~553, 555~557, 559~561 may need to run twice to output correct plots.
