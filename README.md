...
* Plexiform neurofibromas (PN) are embryonic tumors predisposed with loss of function of the NF1 gene in children and young people. PN are benign tumors but have an increased chance of transforming into malignant peripheral nerve sheath tumors. Besides the recent FDA approved drug, SUNITINIB, treatment options are very limited. Drug screen using the immortalized PN cell lines provides an efficient way to obtain a pool of drug candidates. However, there are multiple challenges to scrutinize the candidates before clinical trials: 1) in vitro screen has a different tumor microenvironment from in vivo study, which may void the preclinical studies, 2) a new chemical has an unknown mechanism and toxicity, 3) a well-studied drug commonly has numerous targets and side-effects, 4) it is hard to evaluate the possibility of the candidate to use as a combination with others. 
